...
print("\nCreating balanced ciprofloxacin dataset for E.coli")
...
R_FILE = os.path.join(BASE_DIR, "cleaned_data", "ecoli_R_clean.csv")
S_FILE = os.path.join(BASE_DIR, "cleaned_data", "ecoli_S_clean.csv")
...
balanced.to_csv(
    os.path.join(out_folder, "ecoli_ciprofloxacin_balanced.csv"),
    index=False
)

# ------------------------------------------------
# Save assembly accessions for genome download
# ------------------------------------------------
balanced["Assembly"].to_csv(
    os.path.join(out_folder, "ecoli_accessions.txt"),
    index=False,
    header=False
)
